...
class Output {
    ...
    private function render_post(\stdClass $post_object, $position = 1)
    {
        ...
        if ( $this->public_options['markup']['custom_html'] ) {
            $data = [
                'id' => $post_id,
                'title' => '<a href="' . $permalink . '" ' . ($post_title_attr !== $post_title ? 'title="' . $post_title_attr . '" ' : '' ) . 'class="wpp-post-title" target="' . $this->admin_options['tools']['link']['target'] . '"><h5 style="margin: 0;">' . $post_title . '</h5></a>',
